    ...
    def handle_overlays(self, page):
        ...
        try:
            ...
            try:
                ...
                if consent_clicked:
                    self.log("Nakładka RODO zamknięta przez JS.")
                    time.sleep(1)
                    consent_dismissed = True
            except Exception as e:
                self.log(f"Błąd sprawdzania zgody (Global): {e}")
                ...
                try:
                    ...
                    if consent_iframe:
                        ...
                        if frame:
                            ...
                            if iframe_clicked:
                                self.log("Nakładka RODO zamknięta (iFrame)...")
                                time.sleep(1)
                                consent_dismissed = True
                except Exception as e:
                    self.log(f"Błąd sprawdzania zgody (iFrame): {e}")
            ...
            for selector in selectors:
                try:
                    ...
                    if clicked:
                        self.log(f"Kliknięto przycisk sesji ({selector})")
                        time.sleep(2)
                        return
                except Exception as e:
                    pass
        except Exception as e:
            self.log(f"Krytyczny błąd w handle_overlays: {e}")
    ...
    def run(self):
        ...
        with sync_playwright() as p:
            ...
            if self.browser_type == "Google Chrome":
                ...
                self.log("Uruchamianie Google Chrome...")
            elif self.browser_type == "DuckDuckGo":
                self.log("Uruchamianie silnika DuckDuckGo (Chromium)...")
            
            try:
                browser = p.chromium.launch(**launch_kwargs)
            except Exception as e:
                self.log(f"Uruchomienie przez kanał/ścieżkę nie powiodło się: {e}")
                self.log("Próbuję uruchomienie awaryjne...")
            ...
            def handle_response(response):
                if "generate_next_word.php" in response.url:
                    try:
                        ...
                        self.log(f"Odpowiedź przechwycona: {self.last_answer}")
                    except Exception as e:
                        pass
            ...
            if "instaling.pl/login" not in current_url and "page=login" not in current_url:
                self.log("Nie na stronie logowania. Przekierowuję na stronę logowania Instaling...")
            ...
            if "/login" in page.url or "page=login" in page.url:
                try:
                    self.log("Wykryto stronę logowania. Sprawdzam nakładki...")
                    self.handle_overlays(page)
                    
                    # Oczekiwanie 5 sekund po RODO
                    self.log("Czekam 5 sekund przed wpisaniem danych logowania...")
                    time.sleep(5)
                    
                    self.log("Sprawdzam czy pola logowania są widoczne...")
                    try:
                        page.wait_for_selector("#log_email", timeout=15000)
                        self.log("Pola logowania znalezione! Zaczynam wpisywać...")
                    except Exception as e:
                        self.log(f"Pole #log_email nie znalezione: {e}. Ponawiam sprawdzanie nakładek...")
                        self.handle_overlays(page)
                    ...
                    try:
                        ...
                        self.log("Hasło wpisane.")
                    except Exception as e:
                        self.log(f"Standardowe wpisywanie nie powiodło się ({e}), próbuję JS fallback...")
                        page.evaluate("""(creds) => {
                            document.querySelector('#log_email').value = creds.login;
                            document.querySelector('#log_password').value = creds.password;
                        }""", {"login": self.login, "password": self.password})
                        self.log("Dane logowania ustawione przez JS fallback.")
                    
                    time.sleep(1)
                    page.click("button.btn-primary.w-100")
                    self.log("Kliknięto przycisk logowania.")
                    page.wait_for_load_state("networkidle")
                except Exception as e:
                    self.log(f"Błąd auto-logowania: {e}")
                    # Take a diagnostic screenshot if it fails
                    try:
                        page.screenshot(path="login_error_diagnostic.png")
                        self.log("Zapisano login_error_diagnostic.png")
                    except:
                        pass
            ...
            while self.running:
                try:
                    ...
                    try:
                        ...
                        if return_clicked:
                            self.log("Sesja zakończona! Kliknięto powrót, zaczynam nową pętlę...")
                            time.sleep(3)
                            continue
                    except Exception:
                        pass
                    ...
                    if "/app/session/app.php" in page.url:
                        ...
                        try:
                            ...
                            if znam_btn and znam_btn.is_visible():
                                self.log("Wykryto nowe słowo! Kliknięto 'Znam'.")
                                time.sleep(1)
                                znam_btn.click()
                                time.sleep(1)
                                continue
                        ...
                        try:
                            ...
                            if pomin_clicked:
                                self.log("Wykryto powiadomienie Premium! Kliknięto 'Pomiń'.")
                                time.sleep(1)
                                continue
                        except Exception:
                            pass
                        ...
                        if answer_input and self.last_answer and answer_input.is_visible() and not answer_input.is_disabled():
                            ...
                            self.log(f"Myślę przez {think_time:.1f}s...")
                            time.sleep(think_time)
                            
                            # Celowy błąd (ok. 10% szans)
                            display_word = self.last_answer
                            if random.random() < 0.10:
                                self.log("Symuluję mały błąd dla kamuflażu...")
                            ...
                            self.log(f"Wpisuję: {display_word}")
            ...
            browser.close()
            self.log("Przeglądarka zamknięta.")
